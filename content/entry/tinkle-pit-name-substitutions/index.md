...
It's common for games with a high score list to check the input from the player against a list of known offensive terms. Tinkle Pit certainly does this, checking mostly for sexual words and replacing them with "sports" (heh), but it also has a bunch of other replacements that are a little surprising:
...
(The broken tile graphics along the score text on the top are due to reloading the savestate to capture screenshots. MAME doesn't fully support states for this hardware.)

Yup, the majority of the name substitutions are references to developers and contemporary games. Note that you can only input a max of 6 characters, but there are 8 characters allotted to the string on the high score menu, so using some of these secret substitutions gets you a special, longer name on the screen. Here's the full list of substitutions, pulled directly from the code:
...
Towards the end, the developers put their own names in. Apparently someone there was a bit of a gourmand: suppon is Chinese soft-shelled turtle, and fugu is puffer fish. The 'tabetai!' that they turn into is Japanese for 'I want to eat it!'.
...
The second problem is that there are 0x56 entries total in the substitution list above, but the code only checks up to 0x45. Which means the remaining 17 entries will never be substituted. Oops.
...
There's one final thing to cover: default substitutions. If you don't feel like entering a name and leave it blank, or fill out all six characters with same letter, it will replace the name with a random selection from a list of 136 Namco games. Here is that list:
...
|KNUCKLE |
|K.HEADS |
|[LUCKY&WD](/entry/lucky-wild-disabled-art-viewers-sound-test-and-round-select)|
|SW.COURT|
|GEKITOU |
|DRAGOON |
|EMERALD.|
...
|X-DAY   |
|QUIZ TOU|
|[NEBULAS.](/entry/nebulas-ray-developer-credits-and-enabling-all-namco-nb-1-test-options)|
|CYBER S.|
|TINKLE P|

Have fun on the name entry screen!
